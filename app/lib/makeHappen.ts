...
import { getSqlFromOpenAI } from './getSqlFromOpenAi'
...
export async function makeHappen(editor: Editor, apiKey: string) {
	...
	try {
		const json = await getSqlFromOpenAI({
			image: dataUrl,
			apiKey,
			text: textFromShapes,
			previousPreviews,
		...
		const startString = '```sql\n'
		const endString = '\n```'

		const start = Math.max(message.indexOf(startString), 0)
		const end = Math.min(message.indexOf('\n```', start + startString.length), message.length)
		const sql = message.slice(start + startString.length, end)
